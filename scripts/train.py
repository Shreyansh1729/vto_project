...
def main(config):
    ...
    tryon_model = TryOnPipeline(
        ...
        unet_subfolder=config['model']['subfolder'],
        controlnet_path=config['model']['controlnet_path']
    )
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)
    
    print("Optimizer setup is disabled for this debug run.")
    trainable_params = [] # No trainable parameters
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        for step, batch in enumerate(progress_bar):
            with torch.no_grad():
                ...
                noise_pred = tryon_model(latents=noisy_latents, timestep=timesteps, encoder_hidden_states=text_embeddings, controlnet_cond=pose_map, cloth_image=cloth_image)
